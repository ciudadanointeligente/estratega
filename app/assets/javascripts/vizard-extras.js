...
$(function(){
    ...
    $(btn_sidebar_left).on('click', function(){
      if ( $(this).css('left') == '300px' ) {
        ...
        $('#btn-left').css('-moz-transform','rotate(180deg)');
      } else {
        $(this).css('left','300px');
        $(sidebar_left).css('display','block');
        $(paper_container).css('left','300px');
        $('#btn-left').css('-moz-transform','rotate(0deg)');
      }
    });
});
